...
import localConfig from '~/configs/apollo/default.config.js'
import errorHandler from '~/plugins/apollo/error-handler.js'
...
export default (ctx, inject) => {
  ...
  <% Object.keys(options.clientConfigs).forEach((key) => { %>
      ...
      <% } else if (typeof options.clientConfigs[key] === 'string') { %>
        <%= key %>ClientConfig = localConfig // require('<%= options.clientConfigs[key] %>')
